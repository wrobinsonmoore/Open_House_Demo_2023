...
public class LineChart_2_spatial extends AppCompatActivity {
    ...
    private int blue_sound_id;
    private int blue_sound_stream_id;
    private int orange_sound_id;
    private int orange_sound_stream_id;
    int priority;
    int loop = -1; // Loop forever
    int blue_sound_loop = -1; // loop forever
    int orange_sound_loop = -1; // loop forever

    private Boolean empty_sound_is_playing = false;
    private Boolean blue_sound_is_playing = false;
    private Boolean orange_sound_is_playing = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.linechart_2_spatial);
        ...
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            // In landscape
            mv.setImageResource(R.drawable.line_chart);
        } else {
            // In portrait
            mv.setImageResource(R.drawable.line_chart);
        }
        ...
        sfg.setOnFingerGestureListener(new SimpleFingerGestures_Mod.OnFingerGestureListener() {
            ...
            public boolean onSwipeUp(int fingers, long gestureDuration, double gestureDistance) {
                if (fingers == 1) {
                    ...
                    Intent back_to_graph_selector = new Intent(LineChart_2_spatial.this, GraphSelector.class);
    ...
    public void monitor(ImageView mv, TextView coord_view, TextView output_view) {
        ...
        blue_sound_id = soundPool.load(this, R.raw.fancy_beep, 1);
        orange_sound_id = soundPool.load(this, R.raw.waves_trim, 1);
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (sfg.finger_count == 0){
                    ...
                    // stop blue sounds
                    soundPool.stop(blue_sound_stream_id);
                    blue_sound_is_playing = false;

                    // stop healing sounds
                    soundPool.stop(orange_sound_stream_id);
                    orange_sound_is_playing = false;
                }
                ...
                if (sfg.finger_count == 1){
                    ...
                    if (f0_index != -1) {
                        ...
                        if (pixel_red == inside_red && pixel_green == inside_green && pixel_blue == inside_blue) {
                            ...
                            if (!orange_sound_is_playing) {
                                if (!spatial_audio_activated) {
                                    // Start playing the sound at normal pitch, from both speakers
                                    orange_sound_stream_id = soundPool.play(orange_sound_id, (float) 1.0, (float) 1.0, priority, orange_sound_loop, (float) 1.0);
                                }
                                else {
                                    // Start playing the sound at normal pitch, from both speakers
                                    orange_sound_stream_id = soundPool.play(orange_sound_id, leftVolume, rightVolume, priority, orange_sound_loop, pitch);
                                }
                                orange_sound_is_playing = true;
                            }

                            // SPATIAL AUDIO constant modification
                            if (spatial_audio_activated) {
                                soundPool.setLoop(orange_sound_stream_id, loop);
                                soundPool.setVolume(orange_sound_stream_id, leftVolume, rightVolume);
                                soundPool.setRate(orange_sound_stream_id, pitch);
                            }
                        ...
                        // If finger is INSIDE BLUE LINE
                            ...
                            if (!blue_sound_is_playing) {
                                if (!spatial_audio_activated) {
                                    // Start playing the sound at normal pitch, from both speakers
                                    blue_sound_stream_id = soundPool.play(blue_sound_id, (float) 1.0, (float) 1.0, priority, blue_sound_loop, (float) 1.0);
                                }
                                else {
                                    // Start playing the sound at normal pitch, from both speakers
                                    blue_sound_stream_id = soundPool.play(blue_sound_id, leftVolume, rightVolume, priority, blue_sound_loop, (float) 1.0);
                                }
                                blue_sound_is_playing = true;
                            }
                            // SPATIAL AUDIO constant modification
                            if (spatial_audio_activated) {
                                soundPool.setLoop(blue_sound_stream_id, blue_sound_loop);
                                soundPool.setVolume(blue_sound_stream_id, leftVolume, rightVolume);
                            }


                            // Start vibrating at a different frequency to the orange line
                            if (!vib.isVibrating()) {
//                                vib.vibrateForever();
                                vib.vibrateAtFrequencyForever(10);
                            }
                        }
